...
def main():
    ...
    with tab1:
        ...
        st.data_editor(
            ...
            column_config={
                "ozs_pct": st.column_config.ProgressColumn("OZS% Intent", format="%.1%", min_value=0.0, max_value=1.0),
    ...
    with tab3:
        ...
        if not regression_data.empty:
            st.data_editor(
                regression_data,
                column_config={
                    "ozs_pct": st.column_config.ProgressColumn("OZS%", format="%.1%", min_value=0.0, max_value=1.0),
                    "pdo": st.column_config.NumberColumn("PDO", format="%.3f"),
